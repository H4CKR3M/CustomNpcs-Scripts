...
var JTunes = (function () { var _JTunes = {};
    ...
    var P = {
        enabled: true,
        requiredDLC: "", // <-- required DLC to be downloaded and added to .minecraft/customnpcs/dlc folder. If unsure, leave blank ""
        quadrants: [
            {
                // Congrejo Island
        ...
        TryStartup: function TryStartup() {
            var API = Java.type("noppes.npcs.api.NpcAPI").Instance();
            var world = API.getIWorlds()[0];
            ...
            var eDummy = { API: API }
            var enabled = this.requiredDLC == "" ? true : DLoad.IsInstalled(eDummy, this.requiredDLC);
            if (enabled)
        ...
        GetSong: function GetSong() { 
            var API = Java.type("noppes.npcs.api.NpcAPI").Instance();
            var world = API.getIWorlds()[0];
            return world.getTempdata().get(this.songKey); 
        },
        SetSong: function SetSong(newSong) {
            var API = Java.type("noppes.npcs.api.NpcAPI").Instance();
            var world = API.getIWorlds()[0];

            world.getTempdata().put(this.songKey, newSong);
            world.getStoreddata().put(this.songKey, newSong);
        },
    };
...
var tick = function (e) {
    JTunes.COPY_tick(e);
    JTunes.INTERNAL_Tick(e);
}; tick[JTunes.AutoHookID] = true; tick.CreatedAt = Date.now();

JTunes.COPY_login = (typeof login === 'function' && !login.hasOwnProperty(JTunes.AutoHookID) && (Date.now() - (login.CreatedAt || Date.now())) < 200) ? login : function () { };
var login = function (e) {
    JTunes.COPY_login(e);
    JTunes.INTERNAL_Login(e);
}; login[JTunes.AutoHookID] = true; login.CreatedAt = Date.now();